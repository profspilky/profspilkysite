#!/usr/bin/env python3
"""
fetch_member_urls.py
--------------------
Обходить fpsu.org.ua-сторінки кожної членської організації,
знаходить зовнішній URL (власний сайт) і виводить оновлені
_GALUZEVI / _TERYTORIALNI у вигляді Python-коду.

Запуск:
    python3 tools/fetch_member_urls.py

Результат: tools/member_urls_result.py
"""
...
_FPSU_TERYT = "https://www.fpsu.org.ua/pro-fpu/chlenski-organizatsiji/teritorialni-ob-ednannya-organizatsij-profspilok"
TERYTORIALNI_RAW: list[dict] = [
    {"name": "Федерація профспілок Вінницької області",                   "url": f"{_FPSU_TERYT}/222-federatsiya-profspilok-vinnitskoji-oblasti.html"},
    {"name": "Федерація профспілок Волинської області",                   "url": f"{_FPSU_TERYT}/221-federatsiya-profspilok-volinskoji-oblasti.html"},
    {"name": "Дніпропетровське обласне об'єднання профспілок",            "url": f"{_FPSU_TERYT}/220-dnipropetrovske-oblasne-ob-ednannya-profspilok.html"},
    {"name": "Донецька обласна рада професійних спілок",                  "url": f"{_FPSU_TERYT}/219-donetska-oblasna-rada-profesijnikh-spilok.html"},
    {"name": "Федерація профспілок Житомирської області",                 "url": f"{_FPSU_TERYT}/218-federatsiya-profspilok-zhitomirskoji-oblasti.html"},
    {"name": "Закарпатська обласна рада профспілок",                      "url": f"{_FPSU_TERYT}/217-zakarpatska-oblasna-rada-profspilok.html"},
    {"name": "Запорізька обласна рада профспілок",                        "url": f"{_FPSU_TERYT}/216-zaporizka-oblasna-rada-profspilok.html"},
    {"name": "Рада профспілок Івано-Франківської області",                "url": f"{_FPSU_TERYT}/215-rada-profspilok-ivano-frankivskoji-oblasti.html"},
    {"name": "Київська міська рада профспілок",                           "url": f"{_FPSU_TERYT}/214-kijivska-miska-rada-profspilok.html"},
    {"name": "Київська обласна рада професійних спілок",                  "url": f"{_FPSU_TERYT}/213-kijivska-oblasna-rada-profesijnikh-spilok.html"},
    {"name": "Федерація профспілок Кіровоградської області",              "url": f"{_FPSU_TERYT}/212-federatsiya-profspilok-kirovogradskoji-oblasti.html"},
    {"name": "Федерація незалежних профспілок Криму",                     "url": f"{_FPSU_TERYT}/211-federatsiya-nezalezhnikh-profspilok-krimu.html"},
    {"name": "Федерація профспілок Луганської області",                   "url": f"{_FPSU_TERYT}/210-federatsiya-profspilok-luganskoji-oblasti.html"},
    {"name": "Об'єднання профспілок Львівщини",                           "url": f"{_FPSU_TERYT}/209-ob-ednannya-profspilok-lvivshchini.html"},
    {"name": "Миколаївська обласна рада профспілок",                      "url": f"{_FPSU_TERYT}/208-mikolajivska-oblasna-rada-profspilok.html"},
    {"name": "Федерація профспілок Одеської області",                     "url": f"{_FPSU_TERYT}/207-federatsiya-profspilok-odeskoji-oblasti.html"},
    {"name": "Полтавська обласна рада профспілок",                        "url": f"{_FPSU_TERYT}/206-poltavska-oblasna-rada-profspilok.html"},
    {"name": "Федерація профспілок Рівненської області",                  "url": f"{_FPSU_TERYT}/205-federatsiya-profspilok-rivnenskoji-oblasti.html"},
    {"name": "Сумська обласна рада професійних спілок",                   "url": f"{_FPSU_TERYT}/204-sumska-oblasna-rada-profesijnikh-spilok.html"},
    {"name": "Тернопільська обласна рада профспілок",                     "url": f"{_FPSU_TERYT}/203-ternopilska-oblasna-rada-profspilok.html"},
    {"name": "Об'єднання профспілок Харківської області",                 "url": f"{_FPSU_TERYT}/202-ob-ednannya-profspilok-kharkivskoji-oblasti.html"},
    {"name": "Херсонська обласна міжгалузева рада профспілок",            "url": f"{_FPSU_TERYT}/201-khersonska-oblasna-mizhgaluzeva-rada-profspilok.html"},
    {"name": "Федерація профспілок Хмельницької області",                 "url": f"{_FPSU_TERYT}/200-federatsiya-profspilok-khmelnitskoji-oblasti.html"},
    {"name": "Федерація профспілок Черкаської області",                   "url": f"{_FPSU_TERYT}/199-federatsiya-profspilok-cherkaskoji-oblasti.html"},
    {"name": "Чернівецька обласна рада профспілок",                       "url": f"{_FPSU_TERYT}/198-chernivetska-oblasna-rada-profspilok.html"},
    {"name": "Федерація профспілкових організацій Чернігівської області", "url": f"{_FPSU_TERYT}/197-federatsiya-profspilkovikh-organizatsij-chernigivskoji-oblasti.html"},
    {"name": "Севастопольська міська рада профспілок",                    "url": f"{_FPSU_TERYT}/196-sevastopolska-miska-rada-profspilok.html"},
]

# Патерн «сайт: http://...» або «www.xxx.ua» у тексті статті
_SITE_TEXT = re.compile(
    r'(?:сайт|веб-сайт|website|web-site)[:\s]+'
    r'(https?://[^\s<>"\']+|www\.[^\s<>"\']+)',
    re.IGNORECASE,
)
...
def fetch_html(url: str, timeout: int = 15) -> str:
    try:
        req = urllib.request.Request(
            url,
            headers={"User-Agent": "Mozilla/5.0 (compatible; FPU/1.0)"},
        )
...
def extract_article_body(html: str) -> str:
    """
    Вирізає лише тіло статті (між div.item-page / article / div.com-content-article-body).
    Якщо не знайдено — повертає весь html.
    """
    # Joomla article content markers
    for marker in ("item-page", "article-body", "com-content-article-body",
                   "article-content", "itemBody", "com_content"):
        idx = html.find(marker)
        if idx != -1:
            # беремо текст від цієї позиції до кінця
            return html[idx:]
    return html


def find_external_url(fpsu_url: str) -> str | None:
    """
    Завантажує fpsu.org.ua-сторінку і шукає ЛИШЕ явно вказаний сайт
    організації у тексті (патерн «сайт: http://...»).
    Якщо не знайдено — повертає None (fpsu.org.ua лишається як fallback).
    """
    html = fetch_html(fpsu_url)
    if not html:
        return None

    body = extract_article_body(html)

    m = _SITE_TEXT.search(body)
    if m:
...
def process_list(items: list[dict], label: str) -> list[dict]:
    results = []
    for item in items:
        url = item["url"]
        name = item["name"]

        if "fpsu.org.ua" in url:
            print(f"  ↳ {name[:65]}...")
            found = find_external_url(url)
            if found:
                print(f"    ✓ {found}")
                results.append({"name": name, "url": found, "_src": url})
            else:
                print(f"    – не знайдено, лишаємо fpsu.org.ua")
...
def write_output(galuzevi: list[dict], terytorialni: list[dict], path: str) -> None:
    lines = [
        '"""Автоматично згенеровано tools/fetch_member_urls.py"""',
        "",
        "_GALUZEVI: list[dict] = [",
    ]
    for item in galuzevi:
        src = item.get("_src", "")
        comment = f"  # scraped from {src}" if src else ""
        lines.append(f'    {{"name": {item["name"]!r},\n     "url": {item["url"]!r}}},{comment}')
    lines += ["]", "", "_TERYTORIALNI: list[dict] = ["]
    for item in terytorialni:
        src = item.get("_src", "")
        comment = f"  # scraped from {src}" if src else ""
        lines.append(f'    {{"name": {item["name"]!r},\n     "url": {item["url"]!r}}},{comment}')
    lines += ["]", ""]
...
if __name__ == "__main__":
    ...
    out = os.path.join(os.path.dirname(__file__), "member_urls_result.py")

    print("=" * 65)
    print("Галузеві профспілки")
    print("=" * 65)
    g_done = process_list(GALUZEVI_RAW, "галузева")

    print()
    print("=" * 65)
    print("Територіальні об'єднання")
    print("=" * 65)
    t_done = process_list(TERYTORIALNI_RAW, "тер")

    write_output(g_done, t_done, out)

    updated_g = sum(1 for o, n in zip(GALUZEVI_RAW, g_done) if o["url"] != n["url"])
    updated_t = sum(1 for o, n in zip(TERYTORIALNI_RAW, t_done) if o["url"] != n["url"])
    print(f"\nГалузеві: +{updated_g} нових зовнішніх URL")
    print(f"Територіальні: +{updated_t} нових зовнішніх URL")
